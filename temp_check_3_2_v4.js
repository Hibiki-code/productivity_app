...
// --- EXPERIENCE DETAIL MODAL & SAVE LOGIC & OPEN DETAIL ---
// Restored on branch restore-experience-detail
...
function handleLocationInput(input) {
    ...
    if (val.includes('maps.google.com') || val.includes('goo.gl/maps') ||
        ...
        if (mapUrlField) mapUrlField.value = val; // Store full URL
...
function autoSaveExperience() {
    ...
    const urlObj = document.getElementById('edm-url'); // Wait, is it edm-url or edm-map-url?
    // Checking previous code: 'const url = document.getElementById('edm-url').value;'
    // But map url was 'edm-map-url'. 'edm-url' is probably the generic "Reference URL" field if existing?
    // Checking HTML... I assume 'edm-url' was a dedicated field. 
    // Let's keep it safe.
    ...
    console.log('Auto Saving Experience:', item.title);
