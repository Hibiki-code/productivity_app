...
/**
 * Setup Roadmap Database Sheets
 */
function setupRoadmapDB() {
  const ss = SpreadsheetApp.openById(CONFIG.SPREADSHEET_ID);

  // 1. DB_Goals
  // Column: id, title, vision, metric_label, metric_target, metric_current, start_date, end_date, status, created_at
  let goalsSheet = ss.getSheetByName(CONFIG.SHEET_NAMES.DB_GOALS);
  if (!goalsSheet) {
    goalsSheet = ss.insertSheet(CONFIG.SHEET_NAMES.DB_GOALS);
    ...
    console.log('Created DB_Goals sheet.');
  }

  // 2. DB_Milestones
  // Column: id, goal_id, date, title, status, notes, created_at
  let milestonesSheet = ss.getSheetByName(CONFIG.SHEET_NAMES.DB_MILESTONES);
  if (!milestonesSheet) {
    milestonesSheet = ss.insertSheet(CONFIG.SHEET_NAMES.DB_MILESTONES);
    milestonesSheet.appendRow([
      'id',
      'goal_id',
      'date',
      'title',
    ...
    console.log('Created DB_Milestones sheet.');
  }

  return 'Roadmap DB Setup Complete';
}
